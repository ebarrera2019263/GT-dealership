...
export function BotonFavorito({
      ...
      <button
        ...
        className={cn(
          ...
          activo
            ...
            : 'border-borde bg-superficie text-tinta hover:border-acento hover:text-acento',
        )}
      >
        {corazon}
        {activo ? t('favoritos.saved') : t('favoritos.save')}
      </button>
    );
  }
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={activo ? t('favoritos.removeFromFav') : t('favoritos.addToFav')}
      aria-pressed={activo}
      className={cn(
        'flex size-11 items-center justify-center rounded-full bg-superficie/90 shadow-sm backdrop-blur transition-[color,transform] duration-200 hover:scale-105 active:scale-90',
