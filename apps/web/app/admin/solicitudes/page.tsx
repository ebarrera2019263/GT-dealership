...
export default function AdminSolicitudesPage() {
  ...
  const cargar = useCallback(
    async (desde: number | null, reset: boolean) => {
      ...
      if (!res.ok) {
        setError(t('admin.solicitudes.loadError'));
        setCargando(false);
        return;
      }
  ...
  async function cambiarEstado(s: Solicitud, nuevo: string) {
    ...
    if (!res.ok) {
      ...
      setError(t('admin.solicitudes.stateChangeError'));
    }
  }
  ...
  return (
    <div>
      <h1 className="font-display text-3xl font-bold tracking-tight">
        {t('admin.solicitudes.title')}
      </h1>
      <p className="mt-1 text-sm text-musgo">{t('admin.solicitudes.subtitle')}</p>

      <div className="mt-4 flex items-center gap-2">
        <label className="text-sm text-musgo" htmlFor="filtro-estado">
          {t('admin.solicitudes.state')}
        </label>
        <select
          id="filtro-estado"
          value={estado}
        ...
        >
          <option value="">{t('admin.solicitudes.all')}</option>
          {ESTADOS.map((e) => (
          ...
          <li key={s.id} className="rounded-lg border border-borde bg-white p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <a href={`/autos/${s.vehiculo.slug}`} className="font-medium hover:text-acento">
                ...
                <p className="cifra mt-1 text-sm text-musgo">
                  {t('admin.solicitudes.summary', {
                    cuota: formatearPrecio(s.cuotaEstimada, 'GTQ'),
                    plazo: s.plazo,
                    enganche: formatearPrecio(s.enganche, 'GTQ'),
                  })}
                </p>
              ...
              <select
                ...
                aria-label={t('admin.solicitudes.stateOfAria', { id: s.id })}
                className="rounded-md border border-borde bg-white px-2 py-1 text-sm focus:border-acento focus:outline-none"
              >
                {ESTADOS.map((e) => (
                  <option key={e} value={e}>
                    {e}
      ...
      {cursor && (
        <div className="mt-4 text-center">
          ...
          >
            {cargando ? t('common.loading') : t('admin.solicitudes.loadMore')}
          </button>
        </div>
      )}
    </div>
